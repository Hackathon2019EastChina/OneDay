(function (global, undefined) {
    ...
    function onDocumentMouseDblclick(event) {
        ...
        let text = prompt("标记名称");
        if (!isEmpty(text)) {
            my_addMark(event, '#9400D3', text);
        }
    }

    function my_addMark(e, color , text) {
        let pos = getMousePos(e);
        console.log('鼠标位置: ', pos.x.toFixed(2), pos.y.toFixed(2));
        console.log('经纬度:', _lon.toFixed(2), _lat.toFixed(2));

        let x = _lon - document.body.clientWidth / pos.x * 3;
        let y = _lat + document.body.clientHeight / pos.y * 5;
